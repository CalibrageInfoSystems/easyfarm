...
public class VendorDetails extends AppCompatActivity {
    ...
    VendorsAdapter recyclerAdapter;
    Toolbar toolbar;
    EditText et_search;
    ...
    private void setViews() {
        ...
        mFarmersList = (List<Farmer>) dataAccessHandler.getfarmerDetails(Queries.getInstance().getprosVendorDetails(), 1);

        android.util.Log.d("FarmerDetails====", mFarmersList.size() + "");

        for (int i=0;i<mFarmersList.size();i++) {

            android.util.Log.d("FarmerUserNames===", mFarmersList.get(i).getFirstname() + "");

        }

        recyclerAdapter = new VendorsAdapter(context,mFarmersList);
    ...
    public void doSearch(String searchQuery) {
        ...
        if (!TextUtils.isEmpty(searchQuery)) {
            ...
            if (recyclerAdapter == null) {
                recyclerAdapter = new VendorsAdapter(this, mList_sorted);
            } else {
